...
export const post: APIRoute = async({ request }) => {
  ...
  // Use non-streaming mode when tools are available for proper execution
  ...
  if (enableMCP && tools && tools.length > 0) {
    ...
    let response = await fetch(`${baseUrl}/v1/chat/completions`, nonStreamOptions) as Response
    ...
    if (data.choices?.[0]?.finish_reason === 'tool_calls' && data.choices?.[0]?.message?.tool_calls) {
      ...
      response = await fetch(`${baseUrl}/v1/chat/completions`, finalOptions) as Response
